...
const AppointmentScreen = ({navigation, route}) => {
    ...
    const getVetList = async () => {
        try {
            let querySnapshot = await getDocs(collection(db, "appointment"));
            ...
            <View style={styles.vet}>
                <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', flexShrink:1}}>
                    ...
                    <View style={{flexDirection:'column', marginLeft:20, alignItems:'baseline', flexShrink:1}}>
                        <Text style={{fontSize:18, fontWeight:'bold'}}>{item.data().doctor}</Text>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.addVetView}>
                <Text  
                    onPress={() => navigation.navigate("AddTime", { extra: extra })} 
                    style={styles.addVetText}>Registrar nuevas horas de atención
                </Text> 
